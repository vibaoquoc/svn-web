...
The F<trac/log> template has been changed to not generate useful output
in HTML comments.  Using the new F<trac/log> template (or merging the
change in by hand) will speed up log browsing.
